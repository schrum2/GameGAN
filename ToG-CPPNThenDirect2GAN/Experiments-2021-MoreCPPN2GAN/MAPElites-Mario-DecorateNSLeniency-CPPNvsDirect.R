...
allData$Type[allData$Type == -1] <- "Empty"
allData$Type[allData$Type == 1] <- "CPPN"
allData$Type[allData$Type == 2] <- "Direct"

outputFile <- str_replace(args[1],"txt","heat.pdf")
pdf(outputFile,height=3.5)  
result <- ggplot(allData, aes(x=decorationBin, y=nsBin, fill=factor(Type))) +
  geom_tile() +
